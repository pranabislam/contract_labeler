    ...
    fetch(highlightJsonUrl)
      ...
      .then(data2 => {
        ...
        for (let i = 0; i < labels_coordinates.length; i++) {
            addHBox(
                labels_coordinates[i]['top'],
                labels_coordinates[i]['start'],
                labels_coordinates[i]['width_added'],
                labels_coordinates[i]['height'],
                labels_coordinates[i]['tagged_sequence'],
                labels_coordinates[i]['preds']
            );
        }
    });
    fetch(allJsonUrl)
